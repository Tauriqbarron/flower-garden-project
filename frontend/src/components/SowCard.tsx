...
export default function SowCard({ flower, onRemove, deleting }: SowCardProps) {
  ...
  const c = colors[flower.timing_color as keyof typeof colors] ?? colors.red;

  const slug = flower.slug || encodeURIComponent(flower.name);

  return (
    <div className={`rounded-lg p-4 border ${c.border} hover:shadow-md transition h-full flex flex-col`}>
      <Link href={`/flowers/${slug}`} className="flex-1 cursor-pointer">
      ...
      <div className="mt-2 pt-2 border-t border-[var(--border-soft)] dark:border-[var(--border)]">
        {onRemove ? (
          ...
          <AddToMyGardenButton
            plantType="flower"
            plantSlug={slug}
            plantName={flower.name}
            sowMonth={flower.optimal_month}
          />
        )}
      </div>
    </div>
