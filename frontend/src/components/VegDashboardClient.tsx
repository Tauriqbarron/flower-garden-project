...
import { fetchVegetableDashboard, VegetableDashboardData } from "@/lib/api";
          ...
          {data.top_storage_life.map((v) => (
            <Link key={v.name} href={`/vegetables/${encodeURIComponent(v.name)}`}>
              <div className="bg-gradient-to-br from-[var(--gold-50)] to-[var(--terracotta-50)] rounded-lg p-4 text-center border border-[var(--gold-100)] hover:shadow-md transition cursor-pointer">
                ...
                <div className="text-xs text-[var(--text-muted)] mt-1">{v.name}</div>
              </div>
            </Link>
          ))}
        </div>
...
function ActionCard({ title, emoji, items, color }: { title: string; emoji: string; items: string[]; color: string }) {
  return (
    <div className="glass-card p-6">
      ...
      {items.length === 0 ? (
        ...
        <div className="flex flex-wrap gap-2">
          {items.map((name) => (
            <Link key={name} href={`/vegetables/${encodeURIComponent(name)}`}>
